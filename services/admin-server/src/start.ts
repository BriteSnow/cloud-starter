...
import { KHOST, PERF_LOG_THRESHOLD_WEB, __version__ } from 'common/conf';
...
async function main() {
	...
	app.use(dseWks('/api').middleware());
	app.use(dseMedia('/api').middleware());
	...
	// start the server
	app.listen(PORT);
	console.log(`--> admin-server (${__version__}) ${KHOST} - listening at ${PORT}`);
}
